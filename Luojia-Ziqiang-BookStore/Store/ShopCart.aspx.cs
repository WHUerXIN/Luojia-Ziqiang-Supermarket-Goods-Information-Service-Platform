...
public partial class ShopCart : System.Web.UI.Page
{
    ...
    public void ST_check_Login()
    {
        ...
        {
            Response.Write("<script>alert('对不起！您还未登录');location='Default.aspx'</script>");
            Response.End();
        }
    }
    public void bind()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                DataColumn column7 = new DataColumn("CoverImage");//图片
                ...
                {
                    strSql = "select BookName,Price,CoverImage from tb_OfficialBook where BookID=" + Convert.ToInt32(drRow["BookID"].ToString());
                    ...
                    drRow["CoverImage"] = dstable.Rows[0][2].ToString();//图片
                    price = float.Parse(dstable.Rows[0][1].ToString());//单价                    
                    count = Int32.Parse(drRow["Num"].ToString());
                    drRow["totalPrice"] = string.Format("{0:f2}", price * count); //总价
                    totalPrice += price * count; //计算合价                    
                    i++;
                }
                this.labTotalPrice.InnerText ="￥"+string.Format("{0:f2}", totalPrice);  //显示所有商品的价格
                this.ShoppingCart.DataSource = dtTable.DefaultView;   //绑定DataList控件
                this.ShoppingCart.DataKeyField = "BookID";
                this.ShoppingCart.DataBind();
            }
        }
    }
    protected void lnkbtnUpdate_Click(object sender, EventArgs e)
    {
        ...
        foreach (DataListItem gvr in this.ShoppingCart.Items)
        {
            TextBox otb = (TextBox)gvr.FindControl("txtNum"); //找到用来输入数量的TextBox控件 
            int count = Int32.Parse(otb.Text);//获得用户输入的数量值
            string BookID = ShoppingCart.DataKeys[gvr.ItemIndex].ToString();//得到该商品的ID代
    ...
    protected void ShoppingCart_ItemCommand(object source, DataListCommandEventArgs e)
    {
